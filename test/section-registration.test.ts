/**
 * dsh-omnisearch — settings.section registration contract.
 *
 * Verifies the client registers EXACTLY ONE `settings.section` entry
 * (id "omnisearch", order 30, localized label) and never registers
...
test("section entry carries id omnisearch, order 30, and the localized label", () => {
  ...
  assert.equal(SECTION_ID, "omnisearch");
...
test("registration is idempotent per apply() call (one entry, no duplicates)", () => {
  ...
  assert.equal(registered[0].entry.id, "omnisearch");
  assert.equal(registered[1].entry.id, "omnisearch");
});
