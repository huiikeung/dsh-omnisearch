...
test("Runtime Wiring: apply(ctx) registers routed providers into ctx.web", async () => {
  ...
  const mockCtx: any = {
    ...
    // dsh-omnisearch merges the tool surfaces of three plugins: the host must
    // register them through ctx.tools and add one system-prompt section.
    tools: {
  ...
  assert.equal(registeredSearchProvider.id, "dsh-omnisearch", "Search provider id must match patch config");
  assert.equal(registeredFetchProvider.id, "dsh-omnisearch-fetch", "Fetch provider id must be dsh-omnisearch-fetch");

  // Merged tool surfaces (dsh-free-search + modsearch).
  const toolNames = registeredTools.map((t: any) => t.name).sort();
  assert.deepEqual(
    toolNames,
    ["advanced_search", "free_search_test", "platform_search"],
    "every merged tool must be registered",
  );
  for (const tool of registeredTools) {
    assert.ok(tool.description && tool.description.length > 20, `${tool.name} needs a model-facing description`);
    assert.ok(tool.parameters && Object.keys(tool.parameters).length > 0, `${tool.name} needs a parameter schema`);
    assert.equal(typeof tool.execute, "function", `${tool.name} needs an execute function`);
  }

  // Engine/status prompt section.
  assert.equal(registeredSections.length, 1);
  assert.equal(registeredSections[0].name, "dsh-omnisearch:engines");
  assert.match(registeredSections[0].text, /Web search engines \(dsh-omnisearch plugin\)/);
  assert.match(registeredSections[0].text, /bing/);
});
