/**
 * Route-level smoke tests: register the /omnisearch/api routes on a mock
...
const deps = {
  ...
  readCredential: async (ref) => {
    if (ref === "DSH_OMNISEARCH_TAVILY") return { configured: true, writable: true, value: "tvly-dev-key-one-1234,tvly-dev-key-two-5678" };
    if (ref === "DSH_OMNISEARCH_EXA") return { configured: true, writable: true, value: "exa-key-0001" };
...
test("credentials/describe never leaks credential values", async () => {
  ...
  const tavily = body.value.credentials["DSH_OMNISEARCH_TAVILY"];
...
test("removing the LAST key writes an empty value (routes hand empty to the writer; the host turns it into unset)", async () => {
  ...
  assert.equal(written.ref, "DSH_OMNISEARCH_TAVILY");
});
