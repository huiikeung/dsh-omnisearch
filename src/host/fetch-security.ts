...
export function validateFetchUrl(rawUrl: string): URL {
  ...
  if (
    ...
    throw new FetchSecurityError(
      "WEB_FETCH_BLOCKED",
      `Access to local/internal host "${hostname}" is blocked for security reasons.`,
    );
  }
  ...
  if (ipVersion === 4) {
    if (isPrivateOrRestrictedIpv4(hostname)) {
      throw new FetchSecurityError(
        "WEB_FETCH_BLOCKED",
        `Access to private/loopback/metadata IP "${hostname}" is blocked for security reasons.`,
      );
    }
  } else if (ipVersion === 6) {
    if (isPrivateOrRestrictedIpv6(hostname)) {
      throw new FetchSecurityError(
        "WEB_FETCH_BLOCKED",
        `Access to private/loopback IPv6 "${hostname}" is blocked for security reasons.`,
      );
    }
  }
