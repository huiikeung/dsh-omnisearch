/**
 * dsh-omnisearch — search executor: account pools + deterministic fallback.
 *
...
import { isKeyless, providerError } from "./providers/types.ts";
...
export const PROVIDER_ID = "dsh-omnisearch";
...
export interface ProviderAdapterLike {
  ...
  search(query: string, maxResults: number | undefined, apiKey: string, baseUrl: string | undefined, contextOrSignal?: AbortSignal | { signal?: AbortSignal; options?: unknown; hints?: unknown }): Promise<{ sources: Array<{ url: string; title?: string; snippet?: string; publishedAt?: string }>; content?: string }>;
  ...
  return {
    ...
    async search(
      request: {
        ...
        maxResults?: number;
        /**
         * Preferred engine for THIS call (the `advanced_search` tool passes its
         * explicit `engine`); defaults to the configured default provider.
         * Used only to build the user-facing "Note:" line.
         */
        preferredProvider?: string;
        /** Set by the tool when the preferred engine was skipped for time filtering. */
        preferredSkippedReason?: "time-filter";
        /** Human label of the requested time window (note text / cache key). */
        timeRangeLabel?: string;
        /**
         * Explicit engine order for THIS call. The advanced_search tool uses it
         * to put time-filter-capable engines first when a `timeRange` is given.
         */
        chainOverride?: string[];
        /**
         * Hint overrides merged over the query-derived hints. The
         * advanced_search tool passes an explicit freshness window here so the
         * adapters filter precisely instead of guessing from the query text.
         */
      ...
      const preferredProvider = request.preferredProvider ?? cfg.defaultProvider;
      const baseChain = fallbackChain({
        defaultProvider: cfg.defaultProvider,
        fallbackOrder: cfg.fallbackOrder,
      });
      const chain = request.chainOverride && request.chainOverride.length > 0
        ? request.chainOverride
        : resolveSearchChain(
            baseChain,
            cfg.searchRoutingPolicy ?? "ordered",
            routingState,
      ...
      const baseHints = extractSearchHints(request.query);
      const searchHints: SearchHints = request.hintsOverride
        ? { ...baseHints, ...request.hintsOverride }
        : baseHints;
      ...
      for (const providerName of chain) {
        ...
        const keyless = isKeyless(adapter);
        if (entries.length === 0 && keyless) {
          // Keyless providers (SearXNG, bing/ddg/anysearch/keenable-mcp)
          // execute with an empty key.
        ...
        while (usable.length > 0) {
          ...
          try {
            ...
            // dsh-free-search semantics: ZERO results counts as a failure so
            // the chain keeps looking instead of answering with nothing.
            if ((outcome.sources?.length ?? 0) === 0) {
              lastError = providerError("invalid-response", `${providerName} returned 0 results`);
              attempts.push({ provider: providerName, outcome: "failed:empty", latencyMs });
              stats.record({ provider: providerName, outcome: "failed:empty", latencyMs });
            ...
            const note = buildSearchNote({
              serving: providerName,
              preferred: preferredProvider,
              attempts,
              skippedReason: request.preferredSkippedReason,
              timeRangeLabel: request.timeRangeLabel,
            });
            const withNote = note
...
export function createFetchProvider(
  ...
  return {
    ...
    available() {
      const cfg = resolveConfig();
      // Built-in generic fetcher is always available as long as dsh-omnisearch is enabled.
      return cfg.enabled;
    },
    async fetch(request: { url: string }, signal?: AbortSignal) {
      ...
      for (const providerName of chain) {
        ...
        let entries = await pools.poolOf(providerName);
        if (entries.length === 0) {
          // Keyless fetch-capable providers run with an empty key.
          if (isKeyless(adapter)) entries = [new PoolEntry("", 0)];
          else continue; // no credentials for this backend
        }
...
// ---------------------------------------------------------------------------
// Fallback notes (merged from dsh-free-search's `Note:` contract)
// ---------------------------------------------------------------------------

/** Human text for a classified failure code. */
...
/** Human text for a chain skip reason. */
const SKIP_TEXT: Record<string, string> = {
  "skipped-no-keys": "no API key configured",
  "skipped-no-healthy-keys": "all keys rejected",
  "skipped-cooldown": "cooling down",
  "skipped-no-adapter": "adapter unavailable",
};

/** Turn an attempt outcome into a short human reason (undefined when success). */
export function describeAttempt(outcome: string): string | undefined {
  if (outcome.startsWith("failed:")) {
    const code = outcome.slice("failed:".length);
    return FAILURE_TEXT[code] ?? code;
  }
  if (outcome.startsWith("skipped-")) return SKIP_TEXT[outcome] ?? outcome.slice("skipped-".length);
  return undefined;
}

/**
 * Build the user-facing note that tells the model which engine actually served
 * the request and why the preferred one did not. Three forms, exactly as
 * dsh-free-search defines them:
 *
 *  (a) preferred skipped BEFORE any attempt for time filtering
 *  (b) preferred was tried and failed, with a concrete reason
 *  (c) preferred did not serve and no failure was recorded
 *
 * Returns undefined when the preferred engine served the results itself.
 */
export function buildSearchNote(opts: {
  ...
  const { serving, preferred, attempts, skippedReason, timeRangeLabel } = opts;
  if (serving === preferred) return undefined;
  if (skippedReason === "time-filter") {
    return `Note: ${preferred} does not support time filtering (timeRange=${timeRangeLabel ?? "custom"}), using ${serving}.`;
  }
  const attempt = attempts.find((a) => a.provider === preferred && a.outcome !== "success");
  const reason = attempt ? describeAttempt(attempt.outcome) : undefined;
  return reason
    ? `Note: ${preferred} unavailable or failed (${reason}), using ${serving}.`
    : `Note: ${preferred} unavailable or failed, using ${serving}.`;
}
