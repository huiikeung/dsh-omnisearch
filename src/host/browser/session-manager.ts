...
import { jarSatisfies, loadCookieJar, toCdpCookies } from "./cookie-jar.ts";
...
export class SessionManager implements NativeBrowserRuntime {
  ...
  private readonly loginFailures = new Map<BrowserPlatform, string>();
   ...
   */
  private async injectJarCookies(session: RunningSession, signal?: AbortSignal): Promise<void> {
    const jar = loadCookieJar(session.profileDir, session.platform);
    if (jar.length === 0) return;
    await session.cdp.send(
      "Network.setCookies",
      { cookies: toCdpCookies(jar) },
      undefined,
      signal,
      10_000,
    );
  ...
  async status(platform: BrowserPlatform): Promise<BrowserSessionStatus> {
    ...
    if (!browser) {
      return {
        ...
        lastError:
          this.loginFailures.get(platform) ??
          (() => {
            try {
              locateBrowser(this.browserChoice);
              return undefined;
    ...
    if (recordedFailure) this.loginFailures.delete(platform);
