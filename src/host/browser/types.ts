...
export interface NativeBrowserRuntime {
   ...
   * supported executable exists. Used by platform/login so a missing browser
   * fails loudly instead of silently producing an unresponsive 登录 button.
   */
  assertBrowserAvailable(): void;
  /**
   * Record a login failure for the next status() report, so an error that
   * happens after platform/login already responded is still visible to the user.
   */
