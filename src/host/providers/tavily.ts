...
export const TAVILY_META = {
  name: "tavily",
  label: "AI 搜索 Tavily",
  description: "AI 优化的网页搜索（分块与深度可调）。无 key 也可用（匿名额度），配 key 额度更高、结果更稳。",
  credSuffix: "TAVILY",
  fetchCapable: true,
  needsBaseUrl: false,
  keyless: true,
...
export const TavilyProvider: ProviderAdapter = {
  ...
  async search(query, maxResults, apiKey, _baseUrl, contextOrSignal) {
    ...
    const res = await fetchWithProxy(TAVILY_SEARCH_URL, {
      ...
      headers: {
        ...
        // Keyless anonymous tier (merged from dsh-free-search): without a key
        // Tavily serves a limited anonymous quota instead of rejecting.
  ...
  async fetch(url, apiKey, _baseUrl, contextOrSignal) {
    ...
    const token = (apiKey ?? "").trim();
    if (!token) throw providerError("auth", "Tavily /extract requires an API key (retryable, chain continues)");
