...
import { parseMcpJson } from "./free-engines.ts";
...
export const ParallelProvider: ProviderAdapter = {
  ...
  async search(query, maxResults, apiKey, _baseUrl, contextOrSignal) {
    ...
    if (!token) return searchParallelKeyless(query, count, hints, signal);
  ...
  async fetch(url, apiKey, _baseUrl, contextOrSignal) {
    ...
    const token = (apiKey ?? "").trim();
    if (!token) throw providerError("auth", "Parallel API key is not configured");
 ...
 */
async function searchParallelKeyless(
  query: string,
  count: number,
  ...
  if (!res.ok) {
    if (res.status === 429) throw providerError("rate-limit", "Parallel MCP anonymous quota exhausted", res.status);
    if (res.status >= 500) throw providerError("server", `Parallel MCP error (HTTP ${res.status})`, res.status);
  ...
  const blocks = (json.result?.content ?? []) as Array<{ type?: string; text?: string }>;
