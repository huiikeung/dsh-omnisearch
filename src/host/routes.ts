...
import {
  ...
  stopVisibleLogin,
  type VisibleInput,
} from "./browser/visible-login.ts";
import { renderVncPage } from "./vnc-page.ts";

/** Per-platform view tokens for the visible-login stream. */
const vncTokens = new Map<BrowserPlatform, string>();
...
async function handleVncStart(_deps: RouteDeps, payload: unknown): Promise<{ ok: boolean; platform: string; token?: string; error?: string }> {
  const platform = (payload as any)?.platform as BrowserPlatform;
  if (platform !== "xiaohongshu" && platform !== "x") throw new Error("unknown platform");
  try {
...
function issueVncToken(platform: BrowserPlatform): string {
...
function vncTokenOk(platform: unknown, token: unknown): boolean {
  if (typeof platform !== "string" || typeof token !== "string") return false;
  const expected = vncTokens.get(platform as BrowserPlatform);
...
async function handleVncStop(_deps: RouteDeps, payload: unknown): Promise<{ ok: true }> {
  const platform = (payload as any)?.platform as BrowserPlatform;
...
async function handleVncInput(_deps: RouteDeps, payload: unknown): Promise<{ ok: boolean; error?: string }> {
  const platform = (payload as any)?.platform as BrowserPlatform;
  if (!vncTokenOk(platform, (payload as any)?.token)) return { ok: false, error: "invalid token" };
  const session = platform === "xiaohongshu" || platform === "x" ? getVisibleSession(platform) : undefined;
  if (!session) return { ok: false, error: "no active login window" };
  try {
    await dispatchInput(session, (payload as any)?.input as VisibleInput);
    return { ok: true };
  } catch (error) {
...
async function handleVncState(_deps: RouteDeps, payload: unknown): Promise<{
  active: boolean;
  frame?: string;
  url?: string;
  ...
  const platform = (payload as any)?.platform as BrowserPlatform;
  if (!vncTokenOk(platform, (payload as any)?.token)) return { active: false, error: "invalid token" };
  const session = platform === "xiaohongshu" || platform === "x" ? getVisibleSession(platform) : undefined;
...
async function handlePlatformCookiesSet(deps: RouteDeps, payload: unknown): Promise<{ saved: number; platform: string }> {
  const platform = (payload as any)?.platform as BrowserPlatform;
  if (platform !== "xiaohongshu" && platform !== "x") throw new Error("unknown platform");
  const raw = (payload as any)?.cookies;
  if (typeof raw !== "string" || raw.trim().length === 0) throw new Error("cookies is required");
  ...
  if (trimmed.startsWith("[")) {
    ...
    if (!Array.isArray(parsed)) throw new Error("cookies JSON must be an array");
  ...
  if (jar.length === 0) throw new Error("no usable cookies found in the input");
...
async function handlePlatformLogin(deps: RouteDeps, payload: unknown): Promise<{ status: string }> {
  ...
  if (platform === "xiaohongshu" || platform === "x") {
    ...
    if (deps.sidebarLogin?.available()) {
      void deps.sidebarLogin.login(platform).catch((error: unknown) => {
        deps.nativeRuntime.noteLoginFailure(
          platform,
    ...
    try {
      ...
      throw new Error(
        `${error instanceof Error ? error.message : String(error)} — 请安装 Edge/Chrome/Chromium，或在设置里指定浏览器可执行文件路径`,
      );
    }
    deps.nativeRuntime.login(platform).catch((error: unknown) => {
      deps.nativeRuntime.noteLoginFailure(
        platform,
        error instanceof Error ? error.message : String(error),
      );
...
export interface RouteDeps {
  ...
  sidebarLogin?: {
    available(): boolean;
    login(platform: BrowserPlatform): Promise<{ ok: boolean; error?: string }>;
  };
...
async function handleConfigGet(deps: RouteDeps): Promise<ConfigView> {
  ...
  return {
    ...
    platformSearchEnabled: (cfg.platformSearchEnabled as Record<string, boolean>) ?? {},
...
async function handleConfigSave(deps: RouteDeps, payload: unknown) {
  ...
  // ---- merged knobs ------------------------------------------------------
  if (typeof p.bingMarket === "string") patch.bingMarket = p.bingMarket;
  if (typeof p.region === "string") patch.region = p.region;
  if (p.safeSearch === "off" || p.safeSearch === "moderate" || p.safeSearch === "strict") patch.safeSearch = p.safeSearch;
  if (typeof p.lang === "string") patch.lang = p.lang;
  if (typeof p.cacheTtlMs === "number" && p.cacheTtlMs >= 0 && p.cacheTtlMs <= 300_000) patch.cacheTtlMs = p.cacheTtlMs;
  if (typeof p.promptSection === "boolean") patch.promptSection = p.promptSection;
