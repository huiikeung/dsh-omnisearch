...
import { Config as PluginConfig, DEFAULT_FALLBACK_ORDER, installConfig, type WebToolsSettings } from "./config.ts";
...
import { credRefOf, getProvider, PROVIDERS, PROVIDER_LIST, quotaOf } from "./providers/index.ts";
...
import { installSearchModeRuntime, SearchModeRuntime, SEARCH_NUDGE_TEXT } from "./search-mode-runtime.ts";
import { createUserMessage } from "@deepseek-ai/dsh-llm";
import { createProviderHealthStore } from "./provider-health.ts";
import { setFreeEngineOptions } from "./free-engine-options.ts";
import { buildCacheKey, SearchCache } from "./search-cache.ts";
import { buildPromptText } from "./prompt-section.ts";
import { TIME_FILTER_ENGINES, createAdvancedSearchTool } from "./tools/advanced-search.ts";
import { createEngineTestTool } from "./tools/engine-test.ts";
import { createPlatformSearchTool } from "./tools/platform-search.ts";
...
import { loginViaMcpBrowser, type ToolExecutor } from "./browser/mcp-login.ts";
import { getDedicatedProfileDir } from "./browser/paths.ts";
import { saveCookieJar } from "./browser/cookie-jar.ts";
...
async function checkVersion(): Promise<VersionCheckView> {
  ...
  if (!RELEASES_API) return fallback;
...
export function apply(ctx: WebToolsContext) {
  ...
  ctx.web.registerSearchProvider(cachedSearchProvider as never);
  ...
  ctx.effect(
    () => installSearchModeRuntime(ctx, undefined, searchModeRuntime, searchModeMessages),
    "dsh-omnisearch: search nudge command",
  );
      ...
      disposers.push(
        sctx.tools.register(
          createAdvancedSearchTool({
            ...
            search: (req, signal) => generalSearchProvider.search(req as never, signal) as never,
            chain: () => {
              const cfg = readConfig();
  ...
  ctx.inject(["systemPrompt"], (sctx) => {
    ...
    if (typeof sctx.systemPrompt?.section !== "function") return;
    let disposeSection: (() => void) | undefined;
    const refresh = () => {
      disposeSection?.();
      disposeSection = undefined;
      if (readConfig().promptSection === false) return;
      const cfg = readConfig();
  ...
  const sidebarLogin = {
    ...
    login: async (platform: "xiaohongshu" | "x"): Promise<{ ok: boolean; error?: string }> => {
      if (!toolExecutor) throw new Error("tool runtime unavailable");
