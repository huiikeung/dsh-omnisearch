...
export async function probeEngine(
  ...
  engine: string,
  query: string,
): Promise<EngineTestEntry> {
  const adapter = deps.adapters[engine];
  if (!adapter) return { engine, status: "fail", error: "unknown engine" };
  if (!deps.isEnabled(engine)) return { engine, status: "fail", error: "disabled in settings" };
  ...
  try {
    ...
    if (outcome.sources.length === 0) return { engine, status: "fail", error: "returned 0 results" };
...
export function createEngineTestTool(deps: EngineTestDeps): WebToolsToolDefinition<EngineTestArgs, EngineTestResult> {
  const allEngines = () => Object.keys(deps.adapters);
  return defineTool<EngineTestArgs, EngineTestResult>({
    name: "free_search_test",
    description:
      "体检所有已配置的搜索引擎，报告当前哪些可用。 Use this to diagnose search failures (rate limits, missing keys, unreachable engines) or to answer 'which engines are available?'.",
    parameters: {
      engines: {
        type: "array",
