...
export async function searchGithub(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
...
export async function searchV2ex(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
  const topics = await getJson("https://www.v2ex.com/api/topics/hot.json", {}, signal);
  const q = query.toLowerCase();
  const matched = Array.isArray(topics)
    ? topics.filter(
        (t: any) => (t.title ?? "").toLowerCase().includes(q) || (t.content ?? "").toLowerCase().includes(q),
      )
    : [];
  return matched.slice(0, maxResults).map((t: any) => ({
...
export async function searchBilibili(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
  const data = await getJson(
...
export async function searchReddit(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
  const data = await getJson(
    `https://old.reddit.com/search.json?q=${encodeURIComponent(query)}&limit=${maxResults}&sort=relevance`,
    { "user-agent": `${UA} (dsh-omnisearch; +https://github.com/A3Boy/dsh-web-tools)` },
    signal,
  );
...
export async function searchHackerNews(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
  const data = await getJson(
...
export async function searchStackOverflow(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
  const data = await getJson(
...
export async function searchWikipedia(query: string, maxResults: number, signal?: AbortSignal, lang?: string): Promise<PlatformHit[]> {
...
export async function searchNpm(query: string, maxResults: number, signal?: AbortSignal): Promise<PlatformHit[]> {
...
export function createPlatformSearchTool(deps: PlatformSearchDeps): WebToolsToolDefinition<PlatformSearchArgs, PlatformSearchResult> {
  return defineTool<PlatformSearchArgs, PlatformSearchResult>({
    name: "platform_search",
    description:
      "在指定平台内搜索（而非公开网页）。 Platforms: github (repositories), v2ex (forum threads), bilibili (videos), reddit (posts), hn (Hacker News discussions), stackoverflow (Q&A), wikipedia (articles), npm (packages). Use it when the user names a platform or wants community discussion rather than general web results.",
    parameters: {
      platform: {
        type: "string",
    ...
    output: {
      ...
      render: (_args, value): WebToolsTextBlock[] => {
        if (value.error) return [{ type: "text", text: `platform_search(${value.platform}) failed: ${value.error}` }];
