...
import { PROVIDER_BRAND, PLATFORM_BRAND, BROWSER_PLATFORM_ICONS } from "./brand.ts";
import {
  providerStatusOf,
  testOutcomeStatus,
  quotaSummary,
...
export function WebToolsSection(props: SectionProps) {
  ...
  const [cookiePanelFor, setCookiePanelFor] = useState<"xiaohongshu" | "x" | null>(null);
  const [cookieDraft, setCookieDraft] = useState("");
  const [cookieSaving, setCookieSaving] = useState(false);
  // Drafts for the merged engine/tool knobs (commit on blur / Enter).
  const [bingMarketDraft, setBingMarketDraft] = useState<string>("");
  ...
  const openLoginWindow = async (platform: "xiaohongshu" | "x") => {
    try {
  ...
  const importCookies = async (platform: "xiaohongshu" | "x") => {
  ...
  const PLATFORM_SEARCH_IDS = ["github", "v2ex", "bilibili", "reddit", "hn", "stackoverflow", "wikipedia", "npm"] as const;
  const togglePlatformSearch = (name: string, enabled: boolean) => {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 720, padding: "4px 0 24px" }}>
      ...
      <section>
        ...
        <SettingsGroup style={{ marginTop: 8 }}>
          ...
          <SettingsRow
            icon={
              <img
                ...
                width={22}
                height={22}
                style={{ borderRadius: 5, flex: "none", background: surface.layer1 }}
              />
            }
          ...
          {/* 手动 Cookie 导入面板（无头 NAS：GUI 无可交互浏览器） */}
          {cookiePanelFor && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", borderTop: `1px solid ${surface.border}` }}>
              ...
              <span style={{ fontSize: 12, color: text.tertiary }}>{t("cookieImportHint")}</span>
              <textarea
                value={cookieDraft}
              ...
              />
              <div style={{ display: "flex", gap: 8 }}>
                <Button size="sm" variant="primary" disabled={cookieSaving} onClick={() => void importCookies(cookiePanelFor)}>
                  {cookieSaving ? t("saving") : t("cookieImportSave")}
                </Button>
                <Button size="sm" variant="ghost" onClick={() => { setCookiePanelFor(null); setCookieDraft(""); }}>
                  {t("cancel")}
                </Button>
              </div>
            </div>
          ...
          {/* platform_search platforms (merged from dsh-free-search) */}
      ...
      <section>
        ...
        <SettingsGroup style={{ marginTop: 8 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "12px 14px" }}>
            <div style={{ fontSize: 12, color: text.tertiary }}>{t("enginesAndToolsHint")}</div>

            {/* Safe search */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 13, fontWeight: 500, color: text.primary }}>{t("safeSearchLabel")}</span>
                <span style={{ fontSize: 12, color: text.tertiary }}>{t("safeSearchHint")}</span>
              </div>
              <SegmentedControl
                options={[
            ...
            {/* Bing market / DuckDuckGo region */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 13, fontWeight: 500, color: text.primary }}>{t("bingMarketLabel")}</span>
                <span style={{ fontSize: 12, color: text.tertiary }}>{t("bingMarketHint")}</span>
              </div>
              <input
                type="text"
                ...
                style={{ width: 140, padding: "4px 8px", fontSize: 13, borderRadius: 6, border: `1px solid ${surface.border}`, background: surface.layer1, color: text.primary }}
              />
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 13, fontWeight: 500, color: text.primary }}>{t("ddgRegionLabel")}</span>
                <span style={{ fontSize: 12, color: text.tertiary }}>{t("ddgRegionHint")}</span>
              </div>
              <input
                type="text"
                value={regionDraft}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 13, fontWeight: 500, color: text.primary }}>{t("cacheLabel")}</span>
                <span style={{ fontSize: 12, color: text.tertiary }}>{t("cacheHint")}</span>
              </div>
              <SegmentedControl
                options={[
                ...
                value={String(config.cacheTtlMs ?? 300000)}
                onChange={(v) => void save({ cacheTtlMs: Number(v) })}
              />
            </div>

            {/* System prompt section */}
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: text.primary, cursor: "pointer" }}>
              <input
                type="checkbox"
              ...
              />
              <span>
                {t("promptSectionLabel")}
                <span style={{ display: "block", fontSize: 12, color: text.tertiary }}>{t("promptSectionHint")}</span>
              </span>
            </label>
          </div>
        </SettingsGroup>
      ...
      <section>
        ...
        {providersOpen && (
        <SettingsGroup dividers="inset" style={{ marginTop: 8 }}>
