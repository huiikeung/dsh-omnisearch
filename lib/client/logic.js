...
export function quotaSourceLabel(t, source) {
    if (!source)
        return "";
    const key = QUOTA_SOURCE_LABEL_KEY[source];
    if (key) {
        const val = t(key);
        if (val && val !== key)
            return val;
...
export function providerStatusOf(p, quota, inOrder = true) {
    ...
    // Keyless engines (merged from dsh-free-search) need neither: they are ready
    // out of the box, which is the whole point of the free engine family.
    const configured = p.keyless === true || (selfHosted ? p.baseUrlConfigured === true : p.keyConfigured);
    if (!configured)
...
export function quotaSummary(t, quota) {
    ...
    if (q.source === "local_estimate" && q.unit === "requests" && q.used !== undefined)
        return t("quotaMetered", { n: q.used.toLocaleString() });
...
export function quotaMetaLine(t, q) {
    ...
    if (kind === "observed_usage" && q.used !== undefined) {
        ...
        if (q.unit === "requests")
            return t("quotaSinceRequests", { n: q.used.toLocaleString() });
