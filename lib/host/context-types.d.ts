...
/** One model-facing content block returned by a tool renderer. */
export interface WebToolsTextBlock {
    type: "text";
    text: string;
}
/**
 * Structural mirror of `@deepseek-ai/dsh-tools`'s tool definition, restated
 * because that package is host-provided (it is not part of a third-party
 * plugin's resolvable install tree). `defineTool` there is a typed identity
 * helper, so a local mirror keeps the same runtime shape and gives us types.
 */
export interface WebToolsToolDefinition<TArgs = Record<string, unknown>, TResult = unknown> {
    name: string;
    description: string;
    parameters: Record<string, unknown>;
    output?: {
        schema?: Record<string, unknown>;
        render?: (args: TArgs, value: TResult) => WebToolsTextBlock[];
    };
    /** Allows concurrent execution (both merged tools are read-only). */
    isConcurrencySafe?: boolean;
    execute: (args: TArgs, ctx?: unknown) => Promise<TResult>;
}
/** The tools service face (`ctx.tools`) — the harness ToolRegistry. */
export interface WebToolsTools {
    register(definition: WebToolsToolDefinition<never, never> | WebToolsToolDefinition<any, any>): () => void;
    /**
     * Same-process tool execution. dsh-omnisearch uses it to drive the DSH sidebar
     * browser (Playwright MCP) for interactive platform login.
     */
    execute(exec: {
        callId: string;
        name: string;
...
/** The systemPrompt service face (`ctx.systemPrompt`). */
export interface WebToolsSystemPrompt {
    /** Add one named prompt section; the returned disposer removes it. */
    section(section: {
        name: string;
        order?: number;
        text: string;
 ...
 */
export declare function defineTool<TArgs = Record<string, unknown>, TResult = unknown>(definition: WebToolsToolDefinition<TArgs, TResult>): WebToolsToolDefinition<TArgs, TResult>;
/**
...
export interface WebToolsContext {
    ...
    tools: WebToolsTools;
    /** System-prompt sections (`ctx.systemPrompt`) — engine/status section. */
