/**
 * dsh-omnisearch — network fetch helper.
 *
...
async function getProxyAgentCtor() {
    ...
    try {
        ...
        proxyAgentCtor = typeof undici.ProxyAgent === "function" ? undici.ProxyAgent : null;
    }
    catch {
        proxyAgentCtor = null;
    }
...
export async function proxyStatus() {
    ...
    if (!configured)
        return { configured: false, degraded: false };
...
export async function fetchWithProxy(url, init) {
    ...
    const Ctor = await getProxyAgentCtor();
    if (Ctor === null)
        return fetch(url, init); // undici missing → plain fetch
