...
import { validateFetchUrl, validateFetchDns, FetchSecurityError, } from "./fetch-security.js";
...
const DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 (DSH-WebTools/GenericFetch)";
...
function classifyContentType(contentTypeHeader) {
    if (!contentTypeHeader)
        return "html"; // default to HTML if unspecified
 ...
 */
async function readStreamBounded(response, maxBytes, signal) {
    if (!response.body) {
 ...
 */
export async function fetchGenericWebPage(initialUrl, optionsOrSignal, legacyCustomFetch) {
    const options = optionsOrSignal && "aborted" in optionsOrSignal
        ? { signal: optionsOrSignal, customFetchWithProxy: legacyCustomFetch }
        : optionsOrSignal ?? { customFetchWithProxy: legacyCustomFetch };
    ...
    if (externalSignal) {
        if (externalSignal.aborted)
            throw new GenericFetchError("WEB_ABORTED", "Fetch aborted by caller");
    ...
    try {
        const validateAndResolveUrl = async (rawUrl) => {
            try {
                ...
                if (controller.signal.aborted) {
                    throw new GenericFetchError(abortCause === "timeout" ? "WEB_TIMEOUT" : "WEB_ABORTED", abortCause === "timeout" ? `Fetch timed out after ${timeoutMs}ms` : "Fetch aborted by caller", { url: rawUrl });
                }
        ...
        while (true) {
            if (controller.signal.aborted) {
                throw new GenericFetchError(abortCause === "timeout" ? "WEB_TIMEOUT" : "WEB_ABORTED", abortCause === "timeout" ? `Fetch timed out after ${timeoutMs}ms` : "Fetch aborted by signal");
            }
            if (hopCount > MAX_REDIRECT_HOPS) {
                throw new GenericFetchError("WEB_FETCH_BLOCKED", `Too many redirects (exceeded limit of ${MAX_REDIRECT_HOPS})`, { url: currentUrl });
            }
            try {
                ...
                throw new GenericFetchError("WEB_NETWORK", `Network error fetching "${currentUrl}": ${err instanceof Error ? err.message : String(err)}`, { url: currentUrl });
            }
        ...
        if (!response.ok) {
            throw new GenericFetchError("WEB_HTTP_ERROR", `HTTP fetch failed with status ${response.status} (${response.statusText || "Error"})`, { statusCode: response.status, url: currentUrl });
        }
        const rawContentType = response.headers.get("content-type") || "";
        const typeKind = classifyContentType(rawContentType);
        if (typeKind === "unsupported") {
            throw new GenericFetchError("WEB_UNSUPPORTED_CONTENT_TYPE", `Unsupported Content-Type "${rawContentType || "unknown"}". Only HTML, text, markdown, json, and xml are supported.`, { statusCode: response.status, url: currentUrl });
        }
        ...
        try {
            ...
            if (parseErr instanceof GenericFetchError)
                throw parseErr;
