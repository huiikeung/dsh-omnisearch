export { detectXhsPageState } from "../../browser/xiaohongshu-page-state.js";
export function extractXhsSearchState() {
    ...
    const resolved = feedsRef.value !== undefined
        ? feedsRef.value
...
export function extractXhsDetailState(noteId) {
    const state = window.__INITIAL_STATE__;
    ...
    if (!entry) {
        entry = Object.values(map).find((v) => v?.note?.noteId === noteId);
    }
    ...
    const images = Array.isArray(note.imageList)
        ? note.imageList
            .map((img) => img?.urlDefault || img?.urlPre || "")
            .filter(Boolean)
        : undefined;
    return {
        available: true,
...
export function extractXhsCommentState(noteId) {
    const state = window.__INITIAL_STATE__;
    const rawMap = state?.note?.noteDetailMap;
    const map = rawMap?.value ?? rawMap?._value ?? rawMap;
    if (!map || typeof map !== "object")
    ...
    let entry = map[noteId];
    if (!entry) {
        entry = Object.values(map).find((value) => {
