 ...
 */
async function searchExaKeyless(query, numResults, hints, signal) {
    ...
    if (!res.ok) {
        if (res.status === 429)
            throw providerError("rate-limit", "Exa MCP anonymous quota exhausted", res.status);
        if (res.status >= 500)
            throw providerError("server", `Exa MCP error (HTTP ${res.status})`, res.status);
 ...
 */
export function buildExaSearchBody(query, numResults, options, hints) {
    const cleanQ = hints?.cleanQuery ? hints.cleanQuery : query;
    const body = {
        query: cleanQ,
...
export const ExaProvider = {
    ...
    async search(query, maxResults, apiKey, _baseUrl, contextOrSignal) {
        ...
        if (!token)
            return searchExaKeyless(query, numResults, hints, signal);
    ...
    async fetch(url, apiKey, _baseUrl, contextOrSignal) {
        ...
        if (!token)
            throw providerError("auth", "Exa API key is not configured (native fetch unavailable keyless)");
        const body = {
            urls: [url],
            text: true,
        };
        ...
        if (!text)
            throw providerError("server", `Exa returned no text content for ${url}`);
        return { text };
    },
};
