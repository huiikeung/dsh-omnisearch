 ...
 */
export function buildFirecrawlSearchBody(query, limit, hints) {
...
export const FIRECRAWL_META = {
    name: "firecrawl",
    label: "抓取搜索 Firecrawl",
    description: "搜索 + 干净正文抓取（markdown）。无 key 也可用，配 key 提高限额。",
...
export const FirecrawlProvider = {
    ...
    async search(query, maxResults, apiKey, _baseUrl, contextOrSignal) {
        ...
        const res = await fetchWithProxy(FIRECRAWL_SEARCH_URL, {
            method: "POST",
            // Keyless anonymous quota (merged from dsh-free-search): no Authorization
            // header at all when no key is configured.
            headers: {
        ...
        const sources = results
            ...
            if (typeof r.markdown === "string" && r.markdown) {
                ...
                text = r.passages.filter((p) => typeof p === "string").join("\n\n").slice(0, 1200);
            }
    ...
    async fetch(url, apiKey, _baseUrl, contextOrSignal) {
        const token = (apiKey ?? "").trim();
        // /v2/scrape still wants an account: retryable auth so the fetch chain
        // falls through to the generic HTTP fetcher instead of aborting.
        if (!token)
            throw providerError("auth", "Firecrawl /scrape requires an API key (chain continues)");
