...
export async function tavilyQuota(apiKey, signal) {
    ...
    if (!res.ok) {
        if (res.status === 401 || res.status === 403)
            throw providerError("auth", `Tavily usage auth failed (HTTP ${res.status})`, res.status);
        if (res.status === 429)
            throw providerError("rate-limit", "Tavily rate limit exceeded (HTTP 429)", res.status);
    ...
    for (const field of ["search_usage", "extract_usage", "crawl_usage", "map_usage", "research_usage"]) {
        ...
        if (typeof v === "number")
            breakdown[field.replace("_usage", "")] = v;
    }
    return {
        supported: true,
        authoritative: true,
