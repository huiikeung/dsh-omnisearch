...
export function setFreeEngineOptions(options) {
    current = { ...options };
}
/** Current effective knobs. */
export function getFreeEngineOptions() {
    return current;
}
...
export function acceptLanguageFor(lang) {
    return (LANG_PROFILES[lang ?? "zh"] ?? LANG_PROFILES.zh).acceptLang;
}
/** Bing market for an explicit market or a language code. */
export function bingMarketFor(market, lang) {
    if (market)
        return market;
    return (LANG_PROFILES[lang ?? "zh"] ?? LANG_PROFILES.zh).market;
}
/** Browser-like UA shared by the HTML engines. */
export const FREE_ENGINE_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0 Safari/537.36";
