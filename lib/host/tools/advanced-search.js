...
import { parseTimeRange, timeRangeToFreshness } from "../time-range.js";
/**
 ...
 */
export const TIME_FILTER_ENGINES = [
    "tavily",
    "exa",
    "keenable",
    "firecrawl",
 ...
 */
export function orderChainForTimeRange(chain, timeCapable, requested) {
    ...
    const skippedReason = requested && !capable.has(requested) ? "time-filter" : undefined;
...
export function applyTimeRangeToHints(hints, parsed) {
    return { ...hints, freshness: timeRangeToFreshness(parsed) };
}
/** Build the `advanced_search` tool definition. */
export function createAdvancedSearchTool(deps) {
    return defineTool({
        name: "advanced_search",
        description: "联网搜索（可指定时间范围）。 Use it when the user asks for results from a specific period: fixed tiers (day|week|month|year), relative values (12h, 3d, 2mo, 1y), or an absolute date (2026-07-01 = published on or after that date). Prefers engines that support date filtering and says which engine actually served the results.",
        parameters: {
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.error)
                    return [{ type: "text", text: `advanced_search failed: ${value.error}` }];
                const header = [
        ...
        async execute(args) {
            ...
            if (!query)
                return { query, count: 0, results: [], error: "query is required" };
            ...
            const ordering = parsed
                ...
                : { chain: configured, preferred: requested ?? deps.defaultProvider(), skippedReason: undefined };
            try {
