...
import { isKeyless, providerError } from "./providers/types.js";
    ...
    return {
        ...
        async search(request, signal) {
            ...
            const chain = request.chainOverride && request.chainOverride.length > 0
                ...
                : resolveSearchChain(baseChain, cfg.searchRoutingPolicy ?? "ordered", routingState);
            const attempts = [];
            let lastError;
            const baseHints = extractSearchHints(request.query);
            const searchHints = request.hintsOverride
...
export function describeAttempt(outcome) {
    if (outcome.startsWith("failed:")) {
        const code = outcome.slice("failed:".length);
        return FAILURE_TEXT[code] ?? code;
    }
    if (outcome.startsWith("skipped-"))
        return SKIP_TEXT[outcome] ?? outcome.slice("skipped-".length);
    return undefined;
}
/**
