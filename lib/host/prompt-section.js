...
export function buildPromptText(input) {
    ...
    return [
        ...
        `Safe search: ${input.safeSearch} · Bing market: ${input.bingMarket} · Language: ${input.lang} · Result cache: ${input.cacheTtlMs > 0 ? `${Math.round(input.cacheTtlMs / 1000)}s` : "off"}.`,
        "",
        "Tools:",
 ...
 */
export function installPromptSection(systemPrompt, deps) {
