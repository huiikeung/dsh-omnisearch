...
export function locateBrowser(choice = "auto", fsModule = fs, platformOverride = process.platform, envOverride = process.env) {
    ...
    if (!isWindows) {
        if (platformOverride === "darwin") {
            ...
            const candidates = [
                ...
                // Chromium is the browser most Linux NAS / server distros ship
                // (fnOS, Debian, Arch, Fedora); it was missing here, which made every
                // platform login fail with "No supported browser found".
                { kind: "chrome", path: "/usr/bin/chromium" },
                { kind: "chrome", path: "/usr/local/bin/chromium" },
                { kind: "chrome", path: "/snap/bin/chromium" },
                { kind: "chrome", path: "/var/lib/flatpak/exports/bin/org.chromium.Chromium" },
            ];
    ...
    const localAppData = envOverride.LOCALAPPDATA ||
    ...
    if (choice === "edge" || choice === "auto") {
        for (const p of edgeCandidates) {
            if (fsModule.existsSync(p))
                return { kind: "edge", executablePath: p };
        }
    }
    if (choice === "chrome" || choice === "auto") {
        for (const p of chromeCandidates) {
            if (fsModule.existsSync(p))
                return { kind: "chrome", executablePath: p };
        }
    }
