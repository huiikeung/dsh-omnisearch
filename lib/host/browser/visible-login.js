...
function killStaleProfileChromium(profileDir) {
    let pids = [];
    try {
...
export async function stopVisibleLogin(platform) {
...
export function getVisibleSession(platform) {
 ...
 */
export async function startVisibleLogin(platform) {
    ...
    const session = { platform, port, pid: child.pid, cdp: undefined, startedAt: Date.now(), url };
    sessions.set(platform, session);
    child.on("exit", () => {
        if (sessions.get(platform) === session)
            sessions.delete(platform);
    });
 ...
 */
export async function captureScreen(session, quality = 55, timeoutMs = 4_000) {
    try {
        const result = (await Promise.race([
            session.cdp.send("Page.captureScreenshot", { format: "jpeg", quality, captureBeyondViewport: false, fromSurface: false }, undefined, undefined, timeoutMs),
            new Promise((_, reject) => setTimeout(() => reject(new Error("screenshot timeout")), timeoutMs + 500)),
        ]));
        return result.data ?? "";
    }
    catch {
...
export async function currentUrl(session) {
    try {
        ...
        if (typeof result.result?.value === "string")
            session.url = result.result.value;
    }
    catch {
...
export async function dispatchInput(session, input) {
    switch (input.type) {
        case "click":
...
function vkOf(key) {
    const map = { Enter: 13, Backspace: 8, Tab: 9, Escape: 27, ArrowLeft: 37, ArrowUp: 38, ArrowRight: 39, ArrowDown: 40 };
    return map[key] ?? 0;
}
/**
 ...
 */
export async function pollCookies(session) {
    const raw = (await session.cdp.send("Network.getCookies", {}));
    const config = PLATFORM_AUTH_CONFIG[session.platform];
    const relevant = (raw.cookies ?? [])
...
export function reapIdleVisibleSessions(now = Date.now()) {
    for (const [platform, session] of sessions) {
        if (now - session.startedAt > SESSION_IDLE_MS)
            void stopVisibleLogin(platform);
    }
}
